...
public class Server implements Runnable {
	...
	public static boolean							isPrintEnable	= true;	// print all
																														// messages (print
																														// function)
	public static Map<Integer, Table>	tables				= null;
	public static int									port					= 4449;
	private static ServerSocket				ss						= null;
	private static boolean						isRunning			= false;

	public Server() {
		if (Server.isRunning == false) // run server if isn't running previous
		{
			runServer();
	...
	public void run() // listening
	{
		...
		while (true) {
			...
			try {
				...
				if (!jchess.server.MD5.encrypt(table.password).equals(password)) {
					print("bad password: " + jchess.server.MD5.encrypt(table.password) + " != " + password);
	...
	public void newTable(int idTable, String password, boolean withObserver, boolean enableChat) // create
																																																// new
																																																// table
	{
