...
public class KnightMoveBehavior extends MoveBehavior {
	...
	public ArrayList<ChessboardField> allMoves() throws Exception {
		...
		if ((Chessboard.isout(newX, newY) == false) && checkPiece(newX, newY)) {
			...
			{
				if (this.m_Chessboard.getKingForColor(Player.colors.white).willBeSafeWhenMoveOtherPiece(this.m_Field, m_Chessboard.getFields()[newX][newY])) {
					list.add(m_Chessboard.getFields()[newX][newY]);
				}
			} else // or black
			{
				if (this.m_Chessboard.getKingForColor(Player.colors.black).willBeSafeWhenMoveOtherPiece(this.m_Field, m_Chessboard.getFields()[newX][newY])) {
					list.add(m_Chessboard.getFields()[newX][newY]);
				}
			}
		}
